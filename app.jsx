...
// ความกว้างถังฮิสโทแกรมแบบ "คงที่" (บาท) — ตรึงไว้เพื่อไม่ให้ช่วงแกน X (ป้ายถัง) เปลี่ยนเวลาข้อมูลอัปเดต
// เปลี่ยนเป็น 100 ได้ถ้าอยากได้แท่งน้อยลงเมื่อยอดใช้จ่ายต่อลูกค้ากระจายกว้าง
const HIST_BIN = 50;

// จัดกลุ่มเป็นช่วงกว้างคงที่ (= HIST_BIN) ขอบช่วงลงตัวกับ HIST_BIN เสมอ (เช่น 50–100, 100–150)
// ใช้ความกว้างคงที่แทนการปรับตามข้อมูล → ถังเดิมอยู่ตำแหน่งเดิมทุกครั้งที่ข้อมูลเปลี่ยน (ไม่สับสน)
function makeHistogram(values) {
  const n = values.length;
  if (!n) return [];
  const width = HIST_BIN;
  const min = Math.min(...values), max = Math.max(...values);
  const start = Math.floor(min / width) * width;                        // ขอบล่างปัดลงให้ลงตัวกับความกว้าง
  const end = Math.max(start + width, Math.ceil(max / width) * width);  // ครอบค่าสูงสุด (อย่างน้อยหนึ่งถัง)
    ...
    <div className={"cd-root" + (theme === "light" ? " light" : "")}>
      ...
      <section className="cd-sync">
        ...
        <div className="sync-right">
          <button className="sync-btn" onClick={() => sync({ notify: true })} disabled={loading}>
